			...
			if(hasFollower(chatId)) {
				if(!message.isCommand()) {
					...
					if(txt != null) {
						...
						} else if(txt.startsWith("/js ")) {
							try {
								String js = txt.substring("/js ".length());
								if(js.startsWith("!")) {
									sendMessageHtml(chatId, "<code>" + mods.getScripts().runConsole(js.substring(1)) + "</code>");
									return;
								}
								js = js.replaceAll("=>", "->");
								sendMessageHtml(chatId, "<code>" + mods.getScripts().runConsole(js) + "</code>");
							} catch (Exception e) {
								sendMessageHtml(chatId, "<code>" + e.getMessage() + "</code>");
							}
						} else if(txt.startsWith("/players")) {
							try {
								StringBuilder msg = new StringBuilder("Players:");
								Groups.player.each(p -> msg.append("\n<code>#" + p.id + "</code> <code>" + Strings.stripGlyphs(Strings.stripColors(p.plainName())) + "</code>"));
								sendMessageHtml(chatId, msg.toString());
							} catch (Exception e) {
								sendMessageHtml(chatId, "<code>" + e.getMessage() + "</code>");
							}
						} else if(txt.startsWith("/kick ")) {
							String args = txt.substring("/kick ".length());
							Player found = Groups.player.find(p -> Strings.stripGlyphs(Strings.stripColors(p.plainName())).equalsIgnoreCase(args));
